...
PATH_TRAIN_WHU = './datasets/WHUCD/tiled_dataset/train'
PATH_TEST_WHU = './datasets/WHUCD/tiled_dataset/test'
PATH_VAL_WHU = './datasets/WHUCD/tiled_dataset/val'

class WHUCD_Dataset(Dataset):
    def __init__(self, cropsize=256, type = 'train', name=False, probability_maps=False, priors = False, augmentations=False, gen_img = False):
        ...
        self.im1 = [os.path.join(path,'images1',im) for im in sorted(os.listdir(os.path.join(path,'images1'))) if im.endswith(".tif")] 
        self.im2 = [os.path.join(path,'images2',im) for im in sorted(os.listdir(os.path.join(path,'images2'))) if im.endswith(".tif")]
        self.lab = [os.path.join(path,'labels',im) for im in sorted(os.listdir(os.path.join(path,'labels'))) if im.endswith(".tif")]
        if probability_maps:
        ...
        if self.augmentations:
            ...
            self.color_transform = v2.Compose([v2.RandomCrop(self.cropsize)])
